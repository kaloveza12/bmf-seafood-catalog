(()=>{
  ...
  const isAdmin=location.pathname.startsWith('/adminbmf30/');
  const button=document.createElement('button');button.className='bmf-install';button.textContent=isAdmin?'📲 ติดตั้ง BMF Admin':'📲 ติดตั้งแอป BMF';button.hidden=!isiOS;document.body.append(button);
  ...
  button.addEventListener('click',async()=>{
    ...
    const box=document.createElement('div');box.className='bmf-install-help';box.innerHTML=`<div class="bmf-install-card"><h2>ติดตั้ง ${isAdmin?'BMF Admin':'BMF'} บน iPhone/iPad</h2><ol><li>กดปุ่มแชร์ <b>□↑</b> ด้านล่าง Safari</li><li>เลื่อนแล้วเลือก <b>เพิ่มไปยังหน้าจอโฮม</b></li><li>กด <b>เพิ่ม</b></li></ol><button>เข้าใจแล้ว</button></div>`;box.querySelector('button').onclick=()=>box.remove();box.onclick=e=>{if(e.target===box)box.remove()};document.body.append(box);
  });
